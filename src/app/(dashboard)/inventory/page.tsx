...
import { 
  Package, Plus, Search, Filter, Edit3, Trash2, History,
...
export default function InventoryPage() {
  ...
  const [formData, setFormData] = useState({
    name: '', sku: '', category: 'Pajisje', price: '', stock_quantity: '', description: '', min_stock_level: '2',
  });
  ...
  const openEditModal = (product: any) => {
    ...
    setFormData({
      name: product.name || '', sku: product.sku || '', category: product.category || 'Pajisje',
      price: product.price?.toString() || '', stock_quantity: product.stock_quantity?.toString() || '',
      description: product.description || '', min_stock_level: product.min_stock_level?.toString() || '2',
    });
    setIsModalOpen(true);
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const newQty = parseInt(formData.stock_quantity) || 0;
    const productData = {
      name: formData.name,
      sku: formData.sku,
    ...
    if (editingProduct) {
      // LOGJIKA E HISTORIKUT GJATE UPDATE
      const oldQty = editingProduct.stock_quantity || 0;
      ...
      updateMutation.mutate(
        ...
        { 
          onSuccess: () => { 
            // Regjistrojmë lëvizjen VETËM nëse sasia ka ndryshuar
            if (diff !== 0) {
              recordMovement.mutate({
                product_id: editingProduct.id,
                quantity_change: diff,
                type: diff > 0 ? 'IN' : 'OUT', // PROVO ME SHKRONJA TE MEDHA
                reason: 'Përditësim Manual'
              } as any);
            }
  ...
  const confirmDelete = () => {
    if (deleteId) {
      deleteMutation.mutate(deleteId, { onSuccess: () => { setDeleteConfirmOpen(false); setDeleteId(null); } });
    }
  };
    ...
    <div className="p-4 md:p-8 space-y-8 animate-in fade-in duration-700 italic relative min-h-screen font-medium">
      ...
      <div className="flex flex-col lg:flex-row gap-4">
        <div className="flex-1 relative group">
          ...
          <input 
            type="text" placeholder="Kërko me Emër ose SKU..." 
                ...
                filteredProducts.map((p: any) => (
                  <tr key={p.id} className="hover:bg-slate-50/80 transition-all group italic font-medium">
                    ...
                    <td className="p-8 text-right space-x-2">
                       <button onClick={() => { setSelectedProductIdForHistory(p.id); setSelectedProductName(p.name); }} className="p-3 bg-slate-50 text-slate-400 rounded-xl hover:bg-slate-900 hover:text-white transition-all shadow-sm"><History size={16}/></button>
          ...
          <div className="bg-white w-full max-w-3xl rounded-[3rem] shadow-2xl overflow-hidden animate-in zoom-in duration-300">
            ...
            <div className="p-8 overflow-y-auto max-h-[60vh]">
              {isLoadingMovements ? (
                ...
                <div className="p-20 text-center font-black text-slate-300 uppercase text-xs tracking-widest">Nuk ka lëvizje</div>
              ) : (
                      ...
                      <tr key={m.id}>
                        ...
                        <td className="py-4">
                          {m.type?.toUpperCase() === 'IN' ? 
                            <span className="text-emerald-600 text-[10px] font-black flex items-center gap-1"><ArrowUpRight size={12}/> HYRJE</span> : 
                            <span className="text-red-600 text-[10px] font-black flex items-center gap-1"><ArrowDownRight size={12}/> DALJE</span>
                          }
                        </td>
                        <td className={`py-4 font-black ${m.type?.toUpperCase() === 'IN' ? 'text-emerald-600' : 'text-red-600'}`}>{m.quantity_change} CP</td>
        ...
        <div className="fixed inset-0 bg-slate-900/80 backdrop-blur-md z-[100] flex items-center justify-center p-6">
          <div className="bg-white w-full max-w-2xl rounded-[3rem] shadow-2xl overflow-hidden animate-in zoom-in duration-300 border border-white/10">
             <div className="bg-[#1a1a1a] p-10 flex justify-between items-center text-white">
               <div>
                 <h2 className="text-2xl font-black uppercase">{editingProduct ? 'Edito' : 'Shto'} Produkt</h2>
                 <p className="text-slate-500 text-[10px] font-black uppercase tracking-widest mt-1">Detajet e Stokut</p>
               </div>
             ...
             <form onSubmit={handleSubmit} className="p-10 space-y-6 font-bold overflow-y-auto max-h-[70vh]">
                <div className="grid grid-cols-2 gap-6">
                  ...
                  <div className="space-y-2"><label className="text-[10px] font-black text-slate-400 uppercase">SKU</label><input required type="text" value={formData.sku} onChange={(e)=>setFormData({...formData, sku: e.target.value})} className="w-full p-5 bg-slate-50 border-2 border-slate-100 rounded-[1.5rem] outline-none focus:border-red-600" /></div>
                  <div className="space-y-2"><label className="text-[10px] font-black text-slate-400 uppercase">Kategoria</label><select value={formData.category} onChange={(e)=>setFormData({...formData, category: e.target.value})} className="w-full p-5 bg-slate-50 border-2 border-slate-100 rounded-[1.5rem] outline-none focus:border-red-600"><option>Pajisje</option><option>Softuer</option><option>Aksesorë</option></select></div>
                  <div className="space-y-2"><label className="text-[10px] font-black text-slate-400 uppercase">Çmimi</label><input required type="number" step="0.01" value={formData.price} onChange={(e)=>setFormData({...formData, price: e.target.value})} className="w-full p-5 bg-slate-50 border-2 border-slate-100 rounded-[1.5rem] outline-none focus:border-red-600" /></div>
                  <div className="space-y-2"><label className="text-[10px] font-black text-slate-400 uppercase">Sasia</label><input required type="number" value={formData.stock_quantity} onChange={(e)=>setFormData({...formData, stock_quantity: e.target.value})} className="w-full p-5 bg-slate-50 border-2 border-slate-100 rounded-[1.5rem] outline-none focus:border-red-600" /></div>
                  <div className="col-span-2 space-y-2"><label className="text-[10px] font-black text-red-600 uppercase">Limiti Minim</label><input required type="number" value={formData.min_stock_level} onChange={(e)=>setFormData({...formData, min_stock_level: e.target.value})} className="w-full p-5 bg-red-50/50 border-2 border-red-100 rounded-[1.5rem] outline-none focus:border-red-600" /></div>
                </div>
